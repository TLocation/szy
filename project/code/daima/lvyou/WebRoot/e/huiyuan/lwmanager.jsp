                 ...
                 $("#btnDelete").click(function(){
			        ...
			        $(".check:checked").each(function(index,domEle){
			             ...
			             $.dialog.confirm("你确定要注销在线留言信息?", function(){
				             window.location.href=encodeURI('<%=SystemParam.getSiteRoot()%>/admin/leavewordmanager.do?forwardurl=/e/huiyuan/lwmanager.jsp&actiontype=delete&id='+id);
						  });
						});
					});
				...
				<tbody>
					...
					<c:forEach var="temleaveword" items="${listleaveword}">
						<tr>
							...
							<td>
								<a class="chakan" href="<%=SystemParam.getSiteRoot() %>/e/huiyuan/lwinfo.jsp?id=${temleaveword.id}&seedid=m1">查看</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
