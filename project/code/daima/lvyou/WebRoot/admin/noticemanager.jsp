...
<html>
	...
	<div class="search-title">
		...
		<div class="description">
			<a
				href="<%=SystemParam.getSiteRoot() %>/admin/noticemanager.do?actiontype=load&seedid=207">新建公告</a>
		</div>
	</div>
	<!-- 搜索控件开始 -->
		...
		<tbody>
			...
			<c:forEach var="temnotice" items="${listnotice}">
				<tr>
					...
					<td><a class="edit"
						href="<%=SystemParam.getSiteRoot() %>/admin/noticemanager.do?actiontype=load&id=${temnotice.id}&seedid=207">修改</a>
						<a class="chakan"
						href="<%=SystemParam.getSiteRoot() %>/e/noticeinfo.jsp?id=${temnotice.id}"  target="_blank">查看</a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
