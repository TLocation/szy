...
<h2>
				    ...
				    <form name="kefangform"  method="post" action="<%=SystemParam.getSiteRoot()%>/admin/kefangmanager.do"  id="kefangForm">
		<table class="grid" cellspacing="1" width="100%">
			<input type="hidden" name="id" value="${id}" />
			<input type="hidden" name="actiontype" value="${actiontype}" />
			<input type="hidden" name="seedid" value="${seedid}" />
			<input type="hidden" name="jdid" value="${jdid}" />
			<input type="hidden" name="errorurl" value="/admin/kefangadd.jsp" />
			<input type="hidden" name="forwardurl"
				value="/admin/kefangmanager.do?actiontype=get&forwardurl=/admin/kefangmanager.jsp" />
			<tr>
			...
			<tr>
				<td align="right">客房名称:</td>
				<td><input name="name" placeholder="客房名称"
					validate="{required:true,messages:{required:'请输入客房名称'}}"
					value="${requestScope.kefang.name}" class="input-txt" type="text"
					id="txtName" /></td>
			</tr>
			<tr>
				<td align="right">价格:</td>
				<td><input name="price" placeholder="价格"
					validate="{required:true,number:true,messages:{required:'请输入价格',number:'请输入正确的价格'}}"
					value="${requestScope.kefang.price}" class="input-txt" type="text"
					id="txtPrice" /></td>
			</tr>
			<tr>
				<td colspan="4">
