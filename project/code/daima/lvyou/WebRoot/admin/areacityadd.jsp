...
<h2>
				    ...
				    <form name="areacityform"  method="post" action="<%=SystemParam.getSiteRoot()%>/admin/areacitymanager.do"  id="areacityForm">
				        <table class="grid" cellspacing="1" width="100%">
						        <input type="hidden" name="id" value="${id}" />
						        <input type="hidden" name="actiontype" value="${actiontype}" />
						         <input type="hidden" name="seedid" value="${seedid}" />
						         <input type="hidden" name="errorurl" value="/admin/areacityadd.jsp" />
						        <input type="hidden" name="forwardurl" value="/admin/areacitymanager.do?actiontype=get&forwardurl=/admin/areacitymanager.jsp" />
						        <tr>
								  <td colspan="4">
								      ${errormsg}
								  </td>
											   ...
											   <td>
												   <input name="cityname" placeholder="城市名" validate="{required:true,messages:{required:'请输入城市名'}}" value="${requestScope.areacity.cityname}" class="input-txt" type="text" id="txtCityname"  />
												</td>
											   </tr>
						   <tr>
						       <td colspan="4">
