	...
	private void agree() {
		...
		try {
			response.getWriter().write(
					new Integer(xinxi.getAgreecount()).toString());
		} catch (IOException e) {
...
//踩
	private void against() {
		...
		try {
			response.getWriter().write(
					new Integer(xinxi.getAgainstcount()).toString());
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void save() {
		...
		try {
			if (style != null && style.equals("admin"))
				response.sendRedirect("xinximanager.do?actiontype=get&seedid=201&lanmuid="
						+ xinxi.getLanmuid());
	...
	public void load() {
		...
		try {
			...
			request.getRequestDispatcher("xinxiadd.jsp").forward(request,
					response);
